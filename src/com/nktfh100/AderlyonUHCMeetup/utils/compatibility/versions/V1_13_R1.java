...
public class V1_13_R1 implements PluginVersion {

	Map<String, BiomeBase> biomeBackup = new HashMap<>();

	public void biomeSwapper() {
		List<BiomeBase> base = (List<BiomeBase>) BiomeBase.REGISTRY_ID.e().collect(Collectors.toList());
		for (BiomeBase b : base) {
			this.biomeBackup.put(((MinecraftKey) BiomeBase.REGISTRY_ID.b(b)).getKey(), b);
		}
	...
	public void swap(BiomeData from, BiomeData to) {
		BiomeBase.REGISTRY_ID.a(from.getId(), new MinecraftKey(to.getKey_1_13().toLowerCase()), this.biomeBackup.get(to.getKey_1_13().toLowerCase()));
	}
