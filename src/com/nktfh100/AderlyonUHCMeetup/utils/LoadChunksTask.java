...
import com.nktfh100.AderlyonUHCMeetup.info.CoordXZ;
import com.nktfh100.AderlyonUHCMeetup.info.WorldFileData;
...
public class LoadChunksTask implements Runnable {
	...
	private int fillAutosaveFrequency = 30;
	private int fillMemoryTolerance = 500;

	public LoadChunksTask(Server theServer, String worldName, int fillDistance, int chunksPerRun, int tickFrequency, boolean forceLoad) {
		...
		// load up a new WorldFileData for the world in question, used to scan region
		// files for which chunks are already fully generated and such
		worldData = WorldFileData.create(world);
		if (worldData == null) {
			UHCMeetup.getInstance().getLogger().info("Something went wrong.");
			this.stop();
			return;
		}
		BorderStage border = UHCMeetup.getInstance().getConfigManager().getBorderStages().get(0);
//		this.border.setRadiusX(border.getRadiusX() + fillDistance);
//		this.border.setRadiusZ(border.getRadiusZ() + fillDistance);
		this.x = CoordXZ.blockToChunk((int) 0);
		this.z = CoordXZ.blockToChunk((int) 0);

		int chunkWidthX = (int) Math.ceil((double) ((border.getSize() + fillDistance + 16) * 2) / 16);
		int chunkWidthZ = (int) Math.ceil((double) ((border.getSize() + fillDistance + 16) * 2) / 16);
		int biggerWidth = (chunkWidthX > chunkWidthZ) ? chunkWidthX : chunkWidthZ; // We need to calculate the reportTarget with the bigger width, since the spiral
																					// will only stop if it has a size of biggerWidth x biggerWidth
		this.reportTarget = (biggerWidth * biggerWidth) + biggerWidth + 1;

		// This would be another way to calculate reportTarget, it assumes that we don't
		// need time to check if the chunk is outside and then skip it (it calculates
		// the area of the rectangle/ellipse)
		// this.reportTarget = (this.border.getShape()) ? ((int) Math.ceil(chunkWidthX *
		// chunkWidthZ / 4 * Math.PI + 2 * chunkWidthX)) : (chunkWidthX * chunkWidthZ);
		// Area of the ellipse just to be safe area of the rectangle

		// keep track of the chunks which are already loaded when the task starts, to
		// not unload them
	...
	public void run() {
		...
		if (pausedForMemory) { // if available memory gets too low, we automatically pause, so handle that. el
								// meetup es mio :V, pero esta parte del codigo
								// es de badlion xd, NOpe. pero hable con un desarrollador, y me paso una parte
								// xd. no :v
		...
		// this is set so it only does one iteration at a time, no matter how frequently
		// the timer fires
		readyToGo = false;
		// and this is tracked to keep one iteration from dragging on too long and
		// possibly choking the system if the user specified a really high frequency
		...
		for (int loop = 0; loop < chunksPerRun; loop++) {
			...
			// every 5 seconds or so, give basic progress report to let user know how it's
			// going
			if (now > lastReport + 5000)
				reportProgress();

			// if this iteration has been running for 45ms (almost 1 tick) or more, stop to
			// take a breather
			...
			// if we've made it at least partly outside the border, skip past any such
			// chunks
			while (!UHCMeetup.getInstance().getConfigManager().getBorderStages().get(0).insideBorder(CoordXZ.chunkToBlock(x) + 8, CoordXZ.chunkToBlock(z) + 8)) {
				if (!moveToNext())
					return;
			}
			insideBorder = true;
			...
			if (!forceLoad) {
				// skip past any chunks which are confirmed as fully generated using our
				// super-special isChunkFullyGenerated routine
			...
			// There need to be enough nearby chunks loaded to make the server populate a
			// chunk with trees, snow, etc.
			// So, we keep the last few chunks loaded, and need to also temporarily load an
			// extra inside chunk (neighbor closest to center of map)
			int popX = !isZLeg ? x : (x + (isNeg ? -1 : 1));
			int popZ = isZLeg ? z : (z + (!isNeg ? -1 : 1));
			world.loadChunk(popX, popZ, false);

			// make sure the previous chunk in our spiral is loaded as well (might have
			// already existed and been skipped over)
			...
			// Store the coordinates of these latest 2 chunks we just loaded, so we can
			// unload them after a bit...
			storedChunks.add(new CoordXZ(popX, popZ));
			storedChunks.add(new CoordXZ(x, z));

			// If enough stored chunks are buffered in, go ahead and unload the oldest to
			// free up memory
	...
	public boolean moveToNext() {
		...
		// keep track of progress in case we need to save to config for restoring
		// progress after server restart
		...
		/*
		 * reference diagram used, should move in this pattern: 8 [>][>][>][>][>] etc.
		 * [^][6][>][>][>][>][>][6] [^][^][4][>][>][>][4][v] [^][^][^][2][>][2][v][v]
...
//			Config.UnStoreFillTask();
	...
	private void reportProgress() {
		...
		if (perc > 100)
			perc = 100;
		sendMessage(reportNum + " more chunks processed (" + (reportTotal + reportNum) + " total, ~" + perc + "%" + ")");
		reportTotal += reportNum;
		reportNum = 0;

		// go ahead and save world to disk every 30 seconds or so by default, just in
		// case; can take a couple of seconds or more, so we don't want to run it too
		// often
		if (fillAutosaveFrequency > 0 && lastAutosave + (fillAutosaveFrequency * 1000) < lastReport) {
	...
	private void sendMessage(String text) {
		// Due to chunk generation eating up memory and Java being too slow about GC, we
		// need to track memory availability
		int availMem = this.availableMemory();

		UHCMeetup.getInstance().getLogger().info(text + " (free mem: " + availMem + " MB)");
		...
		if (availMem < 200) { // running low on memory, auto-pause
			...
			UHCMeetup.getInstance().getLogger().info(text);
			// prod Java with a request to go ahead and do GC to clean unloaded chunks from
			// memory; this seems to work wonders almost immediately
			// yes, explicit calls to System.gc() are normally bad, but in this case it
			// otherwise can take a long long long time for Java to recover memory
			System.gc();
		}
	}
